...
当前仓库已进入 M1/M2 初步制作阶段。入口 DLL 使用 `UnityPlayer.dll` 中的 XInput 动态加载候选名 `xinput1_4.dll`，独立于汉化的 `version.dll` 和 Mod Runtime 的 `xinput1_3.dll`。完整游戏内页面尚未接入。第一版目标是：在游戏内查看服装与发型 Mod、显示它们对应的游戏内目标图标与名称，并修改下次启动时的启用状态。
...
- [x] 建立独立本地 Git 仓库
- [x] 完成产品与技术规划
- [x] 建立独立 `xinput1_4.dll` 入口、XInput 转发与 Runtime API 握手
