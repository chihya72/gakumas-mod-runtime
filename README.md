...
当前仓库已进入 M1/M2 初步制作阶段。入口 DLL 使用游戏根目录 `d3d11.dll` 静态依赖的 `xinput9_1_0.dll`，独立于汉化的 `version.dll` 和 Mod Runtime 的 `xinput1_3.dll`。完整游戏内页面尚未接入。第一版目标是：在游戏内查看服装与发型 Mod、显示它们对应的游戏内目标图标与名称，并修改下次启动时的启用状态。
