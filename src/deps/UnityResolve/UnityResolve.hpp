...
public:
	...
	inline static bool startInit;
	inline static Progress assembliesProgress;
	inline static Progress classProgress;
};
...
//#endif
				...
				try {
                    ...
                    return GakumasMod::UnityResolveCompat::ToUTF8(start_char);
				}
				catch (std::exception& e) {
					std::cout << "String Invoke Error\n";
                    GakumasMod::UnityResolveCompat::ErrorFmt("String Invoke Error: %s", e.what());
					return {};
				}
			}

            [[nodiscard]] auto ToWString() const -> std::wstring {
//#if WINDOWS_MODE
//                if (IsBadReadPtr(this, sizeof(String))) return {};
//				if (IsBadReadPtr(m_firstChar, m_stringLength)) return {};
//#endif
			...
			[[nodiscard]] auto Equals(const std::wstring& newString) const -> bool {
				...
				if (std::memcmp(newString.data(), start_char, length) != 0) return false;
				return true;
			}
