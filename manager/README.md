...
截至 2026-08-12，以下主链已经在目标游戏中完成实机验证：
...
- Runtime 启动时注册全部有效替换候选但仍保持 AssetBundle 懒加载；标准 `SkinnedMeshRenderer` 规则会保存替换前的 Mesh、材质和骨骼绑定。关闭和重新开启的热切换主链已经实机生效，热 ON 后直接回主页颜色即正确（2026-08-09，12 轮 ON/OFF 验证）。错误的底层材质数组 icall 实验 Hook 已撤回；托管 `set_sharedMaterials` / `set_materials` Hook 仍保留兼容性诊断，并用于唤醒延迟热重应用。整对象替换和附加式规则仍按重新加载资源降级。
- 2026-08-12 已用 `hmsz-fuyuko-icu` 实机确认“管理页 ON 时当前无 Renderer”的分支：
  `hotInstances=0` 后请求进入延迟队列，返回主页时资源加载路径完成替换，Renderer 生命周期以
  `alreadyPatched=1` 清队列；不再要求先进一次换装页。`atbm-cstm-0140` 的同分支尚未单独复验。
...
- Runtime 写回、开关视觉校正和标准替换热开关主链均已由实机确认；
- 热 ON 后直接回主页颜色错误已于 2026-08-09 解决（真因是换空间漏搬法线/切线），同会话
  12 轮 ON/OFF 的贴图还原与克隆释放全部配对；底栏星形分隔与橙条长度两个 UI 缺陷已于
  2026-08-02 确认修复；
- 管理页切换时无目标 Renderer 的延迟 ON 已于 2026-08-12 用 `hmsz-fuyuko-icu` 确认；
  `hotInstances=0` 是已排队状态，`alreadyPatched=1` 是资源路径先完成替换后的正常收口；
...
- [`../docs/roadmap.md`](../docs/roadmap.md)：已实现能力、实机状态、当前边界与未解项。
...
2. 用 `atbm-cstm-0140` 复验“当前无 Renderer → ON 排队 → 返回主页自动生效”；
