...
> **本文全篇是合并前的记录。** 管理器曾经是独立的 `xinput9_1_0.dll`，靠 `d3d11.dll` 的
> XInput 导入自动进入进程；现在它和 Runtime 编进同一个 `xinput1_3.dll`，游戏目录里
> **没有** `xinput9_1_0.dll`。下面的入口论证、导出转发约束和逐版哈希都对应当时那个 DLL，
> 保留是为了记住入口是怎么选出来的，不要按本文去部署。当前部署见
> [`../README.md`](../README.md)。
...
`xinput9_1_0.dll` 入口，已取得实机截图与日志。合并前最后一版游戏目录部署版为：
